...
{
    [Authorize(Roles = ApplicationRoles.PediaRoleName)]
    [ApiController]
    ...
    public class StaffsController : BaseController
    {
        ...
        {
            var query = new SearchStaffQuery("", TenantId, UserId, ClinicId, c, p, s, sf, so);
            ...
            [FromBody] AddUserInfo info)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var pharmacyStaff = new ClinicStaff
                    {
                        ClinicId = ClinicId,
                        Staff = new Staff
                        {
            ...
            [FromBody] ManageUserRoleInfo info)
        {
            ...
            var staffIds = await appDbContext.ClinicStaffs.Where(e => e.ClinicId == ClinicId).Select(e => e.StaffId).ToListAsync();
            var adminCount = await appDbContext.UserRoles.Where(e => staffIds.Contains(e.UserId) && e.RoleId == ApplicationRoles.Pedia.Id).CountAsync();

            if (adminCount <= 1)
            {
                if (currentUserRoles.Any(e => e.RoleId == ApplicationRoles.Pedia.Id) && !info.RoleIds.Any(e => e == ApplicationRoles.Pedia.Id))
                {
                    return BadRequest("Removing the last Pedia is not allowed.");
                }
            }
